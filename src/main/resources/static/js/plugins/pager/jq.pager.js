	...
	var ms = {
		...
		fillHtml : function(obj, args) {
			return (function() {
				...
				if (args.current > 1) {
					obj
							.append('<a href="javascript:;" style="font-size: 12px"  class="prevPage0">首页</a>');
				} else {
					obj
							.append('<a href="javascript:;" style="font-size: 12px;display: none" class="prevPage0">首页</a>');
				}
				// 上一页
				if (args.current > 1) {
					obj
							.append('<a href="javascript:;" style="font-size: 12px" class="prevPage">上一页</a>');
				} else {
					obj.remove('.prevPage');
				...
				if (args.current < args.pageCount) {
					obj
							.append('<a href="javascript:;" style="font-size: 12px" class="nextPage">下一页</a>');
				} else {
					obj.remove('.nextPage');
				...
				if (args.current < args.pageCount) {
					obj
							.append('<a href="javascript:;" style="font-size: 12px"  class="prevPage1">尾页</a>');
				} else {
					obj
							.append('<a href="javascript:;" style="font-size: 12px;display: none" class="prevPage1">尾页</a>');
				}
			})();
		},
		// 绑定事件
				...
				$("#zhuan")
						.click(
								function() {
									...
									obj
											.append('<span style="display:none" href="javascript:;" class="current">'
													+ tiao + '</span>');
									var current = parseInt(obj.children(
											"span.current").text());
									// console.log(current);
									// var current=tiao;
									aa = current;
